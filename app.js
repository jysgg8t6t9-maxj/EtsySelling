...
(function () {
  ...
  // budget planned inline inputs — save on every keystroke (never lose data)...
  document.addEventListener("input", e=>{
    const el=e.target.closest('[data-act="set-budget"]'); if(!el) return;
    S.budgets[el.dataset.id]=parseFloat(el.value)||0; save();
  });
  // ...then repaint totals + bars the moment you click out of the box
  document.addEventListener("change", e=>{
    const el=e.target.closest('[data-act="set-budget"]'); if(!el) return;
    S.budgets[el.dataset.id]=parseFloat(el.value)||0; save();
    if(S.ui.view==="budget") viewBudget();
  });
